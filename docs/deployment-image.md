...
> [!NOTE]
> `docker-build-context` sets the directory `docker build` resolves `COPY`/`ADD`
> paths from, independently of where the Dockerfile itself lives
> (`application-path`). Override this if your Dockerfile copies files from
